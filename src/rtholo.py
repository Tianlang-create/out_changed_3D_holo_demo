...
class rtholo(nn.Module):
    ...
    def __init__(self, size, mode='train', feature_size=7.48e-6, distance_range=0.03,
        ...
        self.network1 = NET1()
        if CNNPP == False:
            self.network2 = CNN(input_dim=2, output_dim=1, num_layers=num_layers,
                                num_filters_per_layer=num_filters_per_layer, filter_size=3)
        else:
            self.network2 = CNN_PP(input_dim=8, output_dim=4, num_layers=num_layers,
        ...
        if self.precomputed_H == None:
            self.precomputed_H = propagation_ASM(torch.empty(1, 1, self.size, self.size),
        ...
        if self.mode == 'train':
            ...
            for i in track(range(layers_num)):
                ...
                self.pre_kernel.append(
                    propagation_ASM(torch.empty(1, 1, self.size, self.size), feature_size=[feature_size, feature_size],
    ...
    def forward(self, source, ikk):
        ...
        if self.mode == 'train':
            ...
            recon_field = propagation_ASM(holo_field, self.feature_size, self.wavelength, dis,
                                          precomped_H=self.pre_kernel[ikk])
